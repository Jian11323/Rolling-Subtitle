...
class MessageProcessor:
        ...
        try:
            ...
            if not is_valid:
                ...
                logger.debug(
                    ...
                    f"shock_time={shock_time_str}, remained={int(max_seconds - time_diff)}秒"
                )
            return is_valid
        except Exception as e:
    ...
    def _raw_or_data_has_epi_intensity_key(data: Dict[str, Any]) -> bool:
        """原始或解析结果中是否包含 epiIntensity 字段（仅作展示条件，数值另取）。"""
        raw = data.get("raw_data")
        if isinstance(raw, dict) and ("epiIntensity" in raw or "epi_intensity" in raw):
            return True
        if "epiIntensity" in data or "epi_intensity" in data:
            return True
        return False
    ...
    def _resolve_epi_intensity_value(data: Dict[str, Any]) -> Any:
        raw = data.get("raw_data")
        v = data.get("epi_intensity")
        if v is not None:
            return v
        v = data.get("epiIntensity")
        if v is not None:
            return v
        if isinstance(raw, dict):
            rv = raw.get("epiIntensity")
            if rv is not None:
                return rv
            rv = raw.get("epi_intensity")
            if rv is not None:
                return rv
        return None
    ...
    def _append_epi_intensity_after_depth(
        ...
        """
        深度之后：
        - JMA / cwa-eew：仅当原始或解析载荷中**包含 epiIntensity 字段**时追加「预估最大震度」。
        - 其它预警/速报：仍按解析出的数值追加「预估最大烈度…度」（不要求字段名）。
        """
    ...
    def _format_warning_message(self, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # 对于日本气象厅，特殊处理格式
                ...
                if warn_area_type:
                    message_parts.append(f"【Wolfx 日本气象厅 緊急地震速報 {warn_area_type}】")
                else:
                    message_parts.append("【Wolfx 日本气象厅 緊急地震速報】")
            elif source_type == 'wolfx_sc_eew':
