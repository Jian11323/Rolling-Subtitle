            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    if norm_url == WOLFX_ALL_EEW_URL:
                        for cmd in WOLFX_ALL_EEW_QUERY_COMMANDS:
                            try:
                                await websocket.send(cmd)
                                logger.info(f"[{source_name}] 已发送 Wolfx all_eew 查询指令: {cmd}")
                            except Exception as e:
                                logger.warning(f"[{source_name}] 发送 Wolfx 查询指令失败({cmd}): {e}")
                    elif norm_url == WOLFX_CWA_EEW_URL:
                        for cmd in WOLFX_CWA_EEW_QUERY_COMMANDS:
                            try:
                                await websocket.send(cmd)
                                logger.info(f"[{source_name}] 已发送 Wolfx cwa_eew 查询指令: {cmd}")
                            except Exception as e:
                                logger.warning(f"[{source_name}] 发送 Wolfx 查询指令失败({cmd}): {e}")
