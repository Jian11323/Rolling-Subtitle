...
class MainWindow(QMainWindow):
    ...
    def __init__(self, config=None):
        ...
        # 分别存储预警和速报消息
        self.warning_buffer = MessageBuffer(max_size=_mb, use_priority=True)
        self.report_buffer = MessageBuffer(max_size=_mb, use_priority=True)
        ...
        self._move_save_timer.timeout.connect(self._save_window_position_to_config)
    ...
    def _save_window_position_to_config(self):
        """将当前主窗口左上角坐标写入配置"""
        try:
            x = max(-32000, min(32000, self.x()))
            y = max(-32000, min(32000, self.y()))
            g = self.config.gui_config
            if getattr(g, "window_x", -1) == x and getattr(g, "window_y", -1) == y:
                return
            g.window_x = x
            g.window_y = y
            self.config.save_config()
            logger.debug(f"主窗口位置已写入配置: ({x}, {y})")
        except Exception as e:
            logger.error(f"保存窗口位置到配置失败: {e}")
    
    def _persist_window_geometry_to_config(self):
        """关闭前同步写入当前窗口位置与尺寸（单次保存）"""
        try:
            ...
            changed = (
                g.window_width != w
                or g.window_height != h
                or getattr(g, "window_x", -1) != x
                or getattr(g, "window_y", -1) != y
            )
            if not changed:
                return
            g.window_width = w
            ...
            g.window_y = y
            self.config.save_config()
            logger.debug(f"主窗口几何已写入配置: ({x},{y}) {w}x{h}")
        except Exception as e:
            logger.error(f"保存窗口几何到配置失败: {e}")
    ...
    def _show_changelog_if_needed(self):
        ...
        try:
            ...
            declaration = QLabel(APP_DECLARATION_TEXT)
            declaration.setWordWrap(True)
            declaration.setAlignment(Qt.AlignLeft | Qt.AlignTop)
            ...
            sep_line.setStyleSheet("background-color: #e0e0e0; max-height: 1px;")
            ...
            if _dh > 0:
                declaration.setMaximumHeight(_dh)
            content.setFixedWidth(inner_w)
            ...
            dlg.adjustSize()
            max_dlg_h = int(geo.height() * 0.92)
            if dlg.height() > max_dlg_h:
                shrink = dlg.height() - max_dlg_h + 8
    ...
    def _warning_display_segments(
        ...
        try:
            ...
            return build_warning_hint_segments(
                self.config, pd, None, message.text or ""
            )
        except Exception as e:
    ...
    @staticmethod
    def _display_type_is_warning_or_test(t: Optional[str]) -> bool:
        """是否为预警展示态：正式 warning（含书写为 Warning）、设置页模拟 test。"""
        if not isinstance(t, str) or not t:
            return False
        return t.lower() in ("warning", "test")

    @staticmethod
    def _display_mode_like_report_for_incoming_warning(t: Optional[str]) -> bool:
        """与速报轮播相同、收到真预警时可打断的模式（含 ``test`` 模拟态）。"""
        if t is None:
            return True
        if isinstance(t, str):
            return t.lower() in ("report", "test")
        return False

    def _stop_alert_flash_sync(self) -> None:
        """结束告警序列并关闭左侧「地震预警」条（与预警过期、warning→report 切换一致）。"""
        if self.alert_controller is not None:
            try:
                self.alert_controller.cancel()
            except Exception:
                pass
        if self.scrolling_text is not None:
            try:
                fn = getattr(
                    self.scrolling_text, "set_lead_earthquake_badge_flashing", None
                )
                if fn is not None:
                    fn(False)
            except Exception:
    ...
    def _assign_current_display_type(self, new_type: Optional[str]) -> None:
        """
        统一写入 current_display_type。

        由预警展示态（``warning`` / ``Warning`` / ``test``）切到速报轮播（``report``）时，同步
        结束 AlertController 并关闭左侧「地震预警」条（与发震时间/展示侧过期后切速报一致，不因缓冲区仍有多条而保留闪烁）。
        """
        prev = getattr(self, "current_display_type", None)
        if self._display_type_is_warning_or_test(prev) and new_type == "report":
            self._stop_alert_flash_sync()
        self.current_display_type = new_type

    def _on_scroll_completed(self):
        """滚动完成回调"""
        try:
            # 提示期：本条分段字幕滚完一周则结束闪烁并切纯条文；已处理则不再走下方轮播（避免盖掉刚切的纯条文）
            if self.alert_controller is not None:
                if self.alert_controller.consume_scroll_completed_for_hint_phase():
                    return
            # 优先检查是否有预警消息
            if self.warning_buffer.size() > 0:
                ...
                if next_msg:
                    ...
                    if not self._is_warning_still_valid(next_msg):
                        # 预警消息已过期，移除并切换到速报模式
                        logger.info(f"预警消息已过期，移除并切换到速报模式: {next_msg.text}")
                    ...
                    if success:
                        ...
                        logger.info(f"【当前显示-预警】{next_msg.text} | 预警缓冲区: {self.warning_buffer.size()}")
                ...
                # 预警缓冲区已空、轮播切到速报：同步关闪烁（含模拟 test、过期后仍亮条）
                if self._display_type_is_warning_or_test(self.current_display_type):
                    self._stop_alert_flash_sync()
                ...
                if next_msg:
                    ...
                    if success:
                        ...
                        self._current_displaying_message = next_msg
                        logger.info(f"【当前轮播】{next_msg.text} | 数据源: {next_msg.source} | 缓冲区大小: {self.report_buffer.size()}")
            ...
            if self.scrolling_text and message:
                ...
                if success:
                    ...
                    if message.message_type == 'warning' and message.first_displayed_at is None:
                        message.first_displayed_at = time.time()
                    self._pending_update_message = None  # 清除待更新消息
                    logger.info(f"【当前显示-预警】{message.text} | 预警缓冲区: {self.warning_buffer.size()}")
                    ...
                    # 即使更新失败，也记录日志，但保持状态
                    logger.warning(f"预警消息更新失败（可能正在滚动）: {message.text}，将在下次滚动完成时重试")
                    # 确保状态正确，即使更新失败
                    self.current_display_type = 'warning'
                    self._current_displaying_message = message
        except Exception as e:
    ...
    def _switch_to_report_mode(self):
        ...
        try:
            if self._display_type_is_warning_or_test(self.current_display_type):
                # 从预警模式（含设置页 test 模拟态）切换到速报模式，应该从第一条消息开始
                # 防止重复调用
        ...
        try:
            if current_msg and self.scrolling_text:
                ...
                if success:
                    ...
                    logger.warning(f"速报消息更新失败: {current_msg.source} - {current_msg.text} (正在滚动: {is_scrolling})")
        except Exception as e:
    ...
    def on_message_received(self, source_name: str, parsed_data: Dict[str, Any]):
        ...
        try:
            ...
            if isinstance(parsed_data, dict) and "intensity_result" in parsed_data:
                try:
            ...
            color = self.message_processor.get_message_color(
                message_type,
                parsed_data if message_type in ('weather', 'report') else None
            )
            ...
            if self.message_queue.put(msg_item, block=False):
                ...
                if message_type == 'warning':
                    logger.info(f"收到新预警消息 【{source_name}】: {message}")
                elif message_type == 'weather':
                    logger.info(f"收到新气象预警消息 【{source_name}】: {message}")
            else:
            ...
            try:
                ...
                if new_messages:
                    ...
                    if warning_messages:
                        ...
                        if self._display_mode_like_report_for_incoming_warning(self.current_display_type):
                            from .message_manager import get_source_priority
                            # 先加入缓冲区，确保滚动完成后能继续显示预警（而非切回速报）
                            replace_results = self.warning_buffer.batch_replace_by_source(warning_messages)
                            logger.info(
                                f"预警入缓冲区（打断速报模式）: total={len(warning_messages)}, "
                                f"updated={sum(1 for r in replace_results if r)}, "
                                f"added={sum(1 for r in replace_results if not r)}, "
                                f"buffer_size={self.warning_buffer.size()}, "
                                f"sources={[m.source for m in warning_messages]}"
                            )
                            ...
                            if self.scrolling_text and first_warning:
                                ...
                                logger.info(
                                    f"[速报-打断] 收到预警立即切换: {first_warning.source} | {first_warning.text}"
                                )
                        else:
                            ...
                            for i, msg in enumerate(warning_messages):
                                replaced = results[i] if i < len(results) else False
                                if replaced:
                                    updated += 1
                                    logger.debug(
                                        f"预警数据源【{msg.source}】消息已更新: "
                                        f"event_id={msg.event_id}, shock_time={msg.shock_time}, text={msg.text}"
                                    )
                                else:
                                    added += 1
                                    logger.debug(
                                        f"新增预警数据源【{msg.source}】消息: "
                                        f"event_id={msg.event_id}, shock_time={msg.shock_time}, text={msg.text}"
                                    )
                            logger.info(
                                f"预警入缓冲区（预警模式）: total={len(warning_messages)}, "
                                f"updated={updated}, added={added}, buffer_size={self.warning_buffer.size()}"
                            )
                                ...
                                if need_switch:
                                    ...
                                    if getattr(self.config.message_config, 'show_one_alert_per_received', False):
                                        ...
                                        if first_warning:
                                            self._switch_to_warning_mode(first_warning, force_interrupt=True)
                                            logger.info(
                                                f"[收到预警更新报立即切换] 同事件更新报立即打断并更新: {first_warning.source} | {first_warning.text}"
                                            )
                                    else:
                                        logger.info(
                    ...
                    if all_messages:
                        ...
                        for i, msg in enumerate(all_messages):
                            if update_results[i]:
                                logger.info(f"收到数据源更新 【{msg.source}】: {msg.text}")
            ...
            with self.warning_buffer._lock:
                ...
                for msg in self.warning_buffer.buffer:
                    if self._is_warning_still_valid(msg):
                        ...
                        expired_count += 1
                        logger.debug(f"移除过期预警消息: {msg.text}")
                        # 检查当前显示的消息是否过期
                        if (self._current_displaying_message and 
                ...
                if expired_count > 0:
                    ...
                    if len(self.warning_buffer.buffer) == 0 and self._display_type_is_warning_or_test(
    ...
    def get_data_source_status(self) -> Dict[str, str]:
        """
        获取各数据源连接状态，供设置窗口显示。
        返回 Dict[url, state]，state: connected / connecting / disconnected / unconnected。
        """
        result: Dict[str, str] = {}
        try:
            if self.ws_manager and hasattr(self.ws_manager, 'get_connection_status'):
                result.update(self.ws_manager.get_connection_status())
        except Exception as e:
