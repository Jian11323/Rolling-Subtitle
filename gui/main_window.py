...
class MainWindow(QMainWindow):
    ...
    def _append_trusted_site_intensity_clause(
        ...
        ref_clause = f"；预估本地烈度（参考）{val:.1f}"
        if clause in base or ref_clause in base:
            return base
        return base + clause

    def _refresh_warning_text_site_intensity(self, message: MessageItem) -> None:
        """
        在即将滚动展示前，用当前配置补算/补写公式烈度正文。
        入队时虽已追加，但轮播下一条走 _on_scroll_completed 可能不经 _switch_to_warning_mode；
        用户热更新站点坐标后，缓冲区旧条也可因此带上本地烈度。
        """
        try:
            if str(getattr(message, "message_type", "") or "") != "warning":
                return
            pd = getattr(message, "parsed_data", None)
            if not isinstance(pd, dict):
                return
            ir = pd.get("intensity_result")
            pn0 = str(getattr(ir, "provider_name", "") or "") if ir is not None else ""
            if ir is None or pn0 not in _FORMULA_INTENSITY_PROVIDER_NAMES:
                ir = compute_for_parsed(pd, self.config.alert_config)
            if ir is None:
                return
            pn = str(getattr(ir, "provider_name", "") or "")
            if pn not in _FORMULA_INTENSITY_PROVIDER_NAMES:
                return
            pd["intensity_result"] = ir
            message.text = self._append_trusted_site_intensity_clause(
                message.text or "", ir
            )
        except Exception as e:
            logger.debug(f"_refresh_warning_text_site_intensity 失败: {e}")
            ...
            if self.warning_buffer.size() > 0:
                ...
                if next_msg:
                    ...
                    if next_msg.message_type == "warning":
                        self._refresh_warning_text_site_intensity(next_msg)
                    warn_seg = (
        ...
        try:
            if self.scrolling_text and message:
                ...
                if message.message_type == "warning":
                    self._refresh_warning_text_site_intensity(message)
